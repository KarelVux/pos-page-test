...
namespace WebApp.Controllers
{
    ...
    public class PicturesController : Controller
    {
        ...
        {
            var vm = await _bll.PictureService.AllAsync();
            return View(vm);
        }
        ...
        public async Task<IActionResult> Details(Guid? id)
        {
            ...
            var element = await _bll.PictureService.FindAsync(id.Value);
        ...
        public async Task<IActionResult> Create(BLL.DTO.Picture vm)
        {
            if (ModelState.IsValid)
            {
                _bll.PictureService.Add(vm);
        ...
        public async Task<IActionResult> Edit(Guid? id)
        {
            ...
            var vm = await _bll.PictureService.FindAsync(id.Value);
            if (vm == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, BLL.DTO.Picture vm)
        {
            if (id != vm.Id)
            {
                return NotFound();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    _bll.PictureService.Update(vm);
                    await _bll.SaveChangesAsync();
                }
                catch (DbUpdateConcurrencyException)
                {
                    if (!(await PictureExists(vm.Id)))
                    {
                        return NotFound();
                    }
                    else
